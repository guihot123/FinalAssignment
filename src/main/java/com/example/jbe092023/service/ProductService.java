...
public class ProductService {
	...
	public List<Product> findByCategoryId(Integer id,Pageable pageable)throws ProductNotFound,ValidateException{
		...
		return productRepository.findByCategoryId(id,pageable).stream().map(x->x).collect(Collectors.toList());

	}

	public Boolean deleteProduct(Integer id) throws ValidateException,ProductNotFound {
		Optional<Product> findProduct = this.productRepository.findById(id);
